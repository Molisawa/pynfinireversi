...
class FileSaverScreenComponent():
    def __init__(self, screen: Screen, board: Board, menu: Menu, filename: Filename, frame_counter: int, num_of_chars: int) -> None:
        self.board = board
    ...
    def __draw_background_and_calculate(self)->None:
        ...
        self.width = max(measure_text(self.screen.filename.get_instance().text, 30), measure_text("XXXXXXXX", 30)) + 30
    ...
    def __draw_text_and_background_rectangles(self)->None:
        ...
        draw_text(self.screen.filename.get_instance().text, int(screen_specs.SCREEN_WIDTH / 2 - self.width / 2 + 10), int(screen_specs.SCREEN_HEIGHT / 2 - 15), 30, BLACK)
    
    def __do_final_checks(self)->None:
        '''This method does the final checks'''	
        if (self.frame_counter // 20) % 2 == 0:
            draw_text("_", int(screen_specs.SCREEN_WIDTH / 2 - self.width / 2 + measure_text(self.screen.filename.get_instance().text, 30) + 10), int(screen_specs.SCREEN_HEIGHT / 2 - 15), 30, BLACK)

        if is_mouse_button_pressed(0) and self.over_save:
            os.makedirs("../saved", exist_ok=True)
            save_file_text(f"saved/{self.screen.filename.get_instance().text}.brd", self.board.save_game())
            self.screen.filename.get_instance().text = ""
            self.screen.filename.get_instance().num_of_chars = 0
